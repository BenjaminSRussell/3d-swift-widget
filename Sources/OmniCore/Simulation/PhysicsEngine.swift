...
public final class PhysicsEngine {
    ...
    private let verletKernel: OmniCore.ComputeKernel
    ...
    /// Executes the fluid simulation frame including interaction and ML detail.
    public func stepFluid(encoder: MTLComputeCommandEncoder, fluid: FluidSystem, interactions: InteractionSystem, detailTexture: MTLTexture? = nil) {
        ...
        // 4. Inject ML Detail (if available)
        if let detailTexture = detailTexture {
            encoder.setBuffer(fluid.heightBuffer, offset: 0, index: 0)
            encoder.setTexture(detailTexture, index: 0)
            var res = SIMD2<UInt32>(UInt32(fluid.gridRes.x), UInt32(fluid.gridRes.y))
            encoder.setBytes(&res, length: 8, index: 1)
            detailKernel.dispatch(encoder: encoder, gridSize: gridSize)
        }
    }
}
