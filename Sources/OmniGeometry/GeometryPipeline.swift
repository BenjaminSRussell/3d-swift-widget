...
public class GeometryPipeline {
    ...
    public init(device: MTLDevice, pixelFormat: MTLPixelFormat) throws {
        // Load the default library from the OmniGeometry bundle
        guard let library = try? device.makeDefaultLibrary(bundle: Bundle.module) else {
            throw NSError(domain: "OmniGeometry", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to load default library from Bundle.module"])
        }
        
        guard let objectFunction = library.makeFunction(name: "terrain_object"),
              let meshFunction = library.makeFunction(name: "terrain_mesh"),
        ...
        guard let icbFunction = library.makeFunction(name: "update_icb") else {
        ...
        guard let ridgeMesh = library.makeFunction(name: "ridgeline_mesh"),
              let ridgeFrag = library.makeFunction(name: "ridgeline_fragment") else {
